...
public class HashTable<K, V> implements Map<K, V> {
    ...
    public final double standardDeviation() {
        /* TO BE COMPLETED */
         int Nbuckets = theArray.length; // cuantas ListPOI hay
        int Nentrys = 0;
        for(int i = 0; i < theArray.length; ++i) {
            Nentrys += theArray[i].size();
        }
        if(Nentrys == 0) return 0.0;
        double media = (double)Nentrys/Nbuckets;
        double desv = 0;
        for(int i = 0; i < Nbuckets ; i++){
            desv += Math.pow(theArray[i].size() - media,2); 
        }
        desv = desv/ Nbuckets;
        desv = Math.sqrt(desv);
        
        return desv /* CHANGE ME! */;
    }
    ...
    public final double avgLookUpCost() {
        /* TO BE COMPLETED */
        int Nentrys = 0;
        double BucketSizes = 0;
        for( int i  = 0 ; i<theArray.length; i++){
            Nentrys += theArray[i].size();
            BucketSizes += Math.pow(theArray[i].size() , 2);

        }
        double result = BucketSizes / Nentrys;
